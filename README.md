...
Results arrive piecemeal. Each batch prints the moment it answers, in the order the answers come rather than the order the batches were asked, and appends one JSON line to `.triviajudge/sweep-journal.jsonl` before the next is read, so a run killed outright still leaves every answered batch on disk. The journal is emptied when a run starts. Ctrl-C or SIGTERM stops the run: no queued batch starts, the summary covers what landed, `--out` is written, and the batches that never answered are named as not judged. A stopped run exits 130, and a run stopped by an exception that escaped a batch prints `run stopped: <exception>` and exits 1, whatever `--check` says. In `--out`, `unjudged` lists the batches with no answer and `stopped` holds why the run stopped, empty on a whole run.

`--baseline` additionally writes every passed line's digest to `sweep-clean.json`, a file of its own beside the gates' caches, as each batch lands, so a stopped run keeps the amnesty for every batch that answered. The markdown gate reads it at `Stop` alongside its own, so a line the sweep passed is a line the gate does not send again. The amnesty is one file: delete it and the whole amnesty is gone. The comment gate at `Stop` runs the pattern screen alone and makes no model call, so it reads no cache and the comment digests a baseline writes have no reader today.
