...
def test_a_pyproject_carrying_no_table_is_read_at_the_defaults(tmp_path: Path) -> None:
    assert (
        config.read(written(tmp_path, "pyproject.toml", '[project]\nname = "x"\n'))
        == config.Settings()
    )
...
def test_the_standalone_file_is_read_when_pyproject_holds_no_table(
    tmp_path: Path,
) -> None:
    written(tmp_path, "pyproject.toml", '[project]\nname = "x"\n')
    assert (
        config.read(written(tmp_path, ".triviajudge.toml", BARE_TABLE)).model
        == "a-model"
    )
...
def test_a_list_becomes_the_type_the_field_holds(tmp_path: Path) -> None:
    ...
    assert (settings.md_skip, settings.excluded) == (
        frozenset({"NOTES.md"}),
        ("tests/",),
    )
...
def test_a_table_that_does_not_parse_refuses_to_judge_on_defaults(
    tmp_path: Path,
) -> None:
...
def test_a_key_nobody_knows_fails_rather_than_passing_in_silence(
    tmp_path: Path,
) -> None:
    with pytest.raises(RuntimeError, match="mdel"):
        config.read(
            written(
                tmp_path, "pyproject.toml", '[tool.triviajudge]\nmdel = "a-model"\n'
            )
        )
