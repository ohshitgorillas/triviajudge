...
NOTHING_MEASURED = (
    "{report}: no <testsuite time=...> in the report; nothing was measured"
)
...
def report_of(tmp_path: Path, attrs: str) -> Path:
    ...
    report.write_text(
        f'<?xml version="1.0"?><testsuites>{element}</testsuites>', encoding="utf-8"
    )
    return report
...
def timed(tmp_path: Path, seconds: float) -> Path:
    ...
    return report_of(
        tmp_path, f'name="pytest" errors="0" failures="0" time="{seconds}"'
    )
...
def test_a_report_that_was_never_written_is_refused(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    report = tmp_path / "absent.xml"
    assert verdict(report, tmp_path / "baseline.json", capsys) == (
        1,
        NO_REPORT.format(report=report),
        None,
    )
...
def test_a_report_stating_no_time_is_refused(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
...
@pytest.mark.parametrize(
    "attrs",
    ['errors="0" failures="2" time="3.0"', 'errors="1" failures="0" time="3.0"'],
)
def test_a_red_suite_is_not_judged_and_records_nothing(
    ...
    assert verdict(report, tmp_path / "baseline.json", capsys) == (
        1,
        SUITE_RED.format(report=report),
        None,
    )
...
def test_a_first_run_is_recorded_as_the_bar(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    assert verdict(timed(tmp_path, 4.0), tmp_path / "baseline.json", capsys) == (
        0,
        SEEDED_4S,
        4.0,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            19.9,
            True,
            (True, "suite 19.9s, last green 10.0s: +9.9s accepted by the owner"),
        ),
        (
            20.0,
            True,
            (False, "suite 20.0s, last green 10.0s: +10.0s, rejected (limit 10s)"),
        ),
        (25.0, False, (False, REJECTED_25S)),
    ],
)
def test_a_run_is_judged_against_the_last_green_run(
    seconds: float, accept: bool, expected: tuple[bool, str]
) -> None:
    assert GATE.judge(seconds, 10.0, accept=accept) == expected


def test_a_run_inside_the_band_becomes_the_new_baseline(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    baseline = baseline_of(tmp_path, 10.0)
    assert verdict(timed(tmp_path, 11.0), baseline, capsys) == (
        0,
        INSIDE_BAND_11S,
        11.0,
    )
...
def test_a_run_the_owner_accepts_becomes_the_new_baseline(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    baseline = baseline_of(tmp_path, 10.0)
    assert verdict(timed(tmp_path, 16.0), baseline, capsys, accept=True) == (
        0,
        ACCEPTED_16S,
        16.0,
    )
...
def test_a_rejected_run_leaves_the_baseline_where_it_was(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    baseline = baseline_of(tmp_path, 10.0)
    assert verdict(timed(tmp_path, 25.0), baseline, capsys, accept=True) == (
        1,
        REJECTED_25S,
        10.0,
    )
...
def test_a_worktree_resolves_to_the_tree_it_was_cut_from(
...
def test_the_cli_judges_this_tree_s_report_and_seeds_beside_the_checkout(
    ...
    assert (code, printed, GATE.read_baseline(tmp_path / GATE.BASELINE_NAME)) == (
        0,
        SEEDED_3S,
        3.0,
    )
