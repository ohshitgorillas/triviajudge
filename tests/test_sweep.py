...
def carries(line: Line, args: tuple[object, ...], kwargs: dict[str, object]) -> bool:
    """Whether one call's arguments hand over ``line``, bare, in a list, or on a batch."""
    for arg in (*args, *kwargs.values()):
        held = getattr(arg, "lines", arg)
        if held == line or (isinstance(held, list | tuple) and line in held):
            return True
    return False
...
@pytest.mark.parametrize(
    ("free", "first"),
    [(LINES[2], "md#2"), (LINES[6], "md#4")],
    ids=["second batch free", "fourth batch free"],
)
def test_every_batch_is_asked_when_the_calls_run_concurrently(
    monkeypatch: pytest.MonkeyPatch, free: Line, first: str
) -> None:
    released = threading.Event()

    def held(*args: object, **kwargs: object) -> list[dict[str, str]]:
        if not carries(free, args, kwargs):
            released.wait(timeout=5)
        return []

    monkeypatch.setattr("triviajudge.sweep.verdicts", held)
    batches = sweep.batched(sweep.MD, "prompt", LINES, 2)
    names: list[str] = []
    for batch, _flags, _why in sweep.run_batches(batches, "a-model", 4):
        released.set()
        names.append(batch.name)
    assert (names[:1], sorted(names)) == (
        [first],
        ["md#1", "md#2", "md#3", "md#4"],
    )
